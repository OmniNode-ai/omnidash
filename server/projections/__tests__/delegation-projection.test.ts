 ...
 *
 * The DelegationProjection._queryForWindow uses Promise.all to run 7 queries
 ...
 *   2: _queryByModel
 *   3: _queryCostSavings
 *   4: _queryQualityGates
 *   5: _queryShadowDivergence
 *   6: _queryTrend
 *   7: _querySummary shadow comparisons (resumes after all single-call queries start)
 *   8: _querySummary agent_actions count
 *   9: _querySummary quality gate trend
 *
...
describe('DelegationProjection', () => {
  ...
  it('should return zeros from getSnapshot when DB is unavailable', () => {
    ...
    expect(snapshot.payload.byModel).toEqual([]);
  ...
  it('should throw for invalid window in ensureFreshForWindow', async () => {
    await expect(projection.ensureFreshForWindow('1h')).rejects.toThrow('invalid window "1h"');
    await expect(projection.ensureFreshForWindow('90d')).rejects.toThrow('invalid window "90d"');
  });
  ...
  it('should compute summary correctly', async () => {
    ...
    allResults[0] = [
      {
        ...
        qg_passed: 8,
        qg_failed: 2,
        total_cost_savings: '50.00',
        avg_cost_savings: '5.00',
        avg_latency: 150,
      },
    ];
    ...
    // 2: _queryByModel's execute
    // 3: _queryCostSavings's execute
    // 4: _queryQualityGates's execute
    // 5: _queryShadowDivergence's execute
    // 6: _queryTrend's execute
    // Then after all resolve (microtask queue), _querySummary resumes:
    // 7: _querySummary's 2nd execute (shadow agg)
    // 8: _querySummary's 3rd execute (action count)
    // 9: _querySummary's 4th execute (qg trend)

    allResults[1] = []; // byTaskType
    allResults[2] = []; // byModel
    allResults[3] = []; // costSavings
    allResults[4] = []; // qualityGates
    allResults[5] = []; // shadowDivergence
    allResults[6] = []; // trend
    allResults[7] = [{ total_shadow: 4, diverged: 1, agreed: 3 }]; // summary shadow
    allResults[8] = [{ action_count: 100 }]; // summary action count
    allResults[9] = [{ date: '2026-02-20', value: 0.8 }]; // summary qg trend
  ...
  it('should compute byTaskType with correct pass rates', async () => {
    ...
    allResults[0] = [
      {
        ...
        qg_passed: 4,
        qg_failed: 1,
        total_cost_savings: '20.00',
        avg_cost_savings: '4.00',
        avg_latency: 100,
      },
    ];
    ...
    // byModel (index 2)
    allResults[2] = [];
    // summary shadow (index 7)
    allResults[7] = [{ total_shadow: 0, diverged: 0, agreed: 0 }];
    // summary action count (index 8)
    allResults[8] = [{ action_count: 50 }];
  ...
  it('should assemble trend data correctly', async () => {
    ...
    allResults[0] = [
      {
        ...
        qg_passed: 3,
        qg_failed: 1,
        total_cost_savings: '10.00',
        avg_cost_savings: '2.50',
        avg_latency: 80,
      },
    ];
    // trend (index 6)
    ...
    allResults[7] = [{ total_shadow: 2, diverged: 1, agreed: 1 }];
    allResults[8] = [{ action_count: 20 }];
