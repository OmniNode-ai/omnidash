/**
 * EffectivenessMetricsProjection — DB-backed projection for effectiveness dashboard (OMN-2325)
 *
 * Encapsulates all SQL queries from effectiveness-routes.ts behind the
 * ProjectionView interface. Routes call getSnapshot() or query individual
 * methods instead of executing SQL directly.
 *
...
export interface EffectivenessMetricsPayload {
...
export class EffectivenessMetricsProjection extends DbBackedProjectionView<EffectivenessMetricsPayload> {
  readonly viewId = 'effectiveness-metrics';

  protected emptyPayload(): EffectivenessMetricsPayload {
    return {
      summary: {
        injection_rate: 0,
  ...
  async queryThrottleStatus(db: Db): Promise<ThrottleStatus> {
    const ie = injectionEffectiveness;
  ...
  async queryLatency(db: Db): Promise<LatencyDetails> {
  ...
  async queryUtilization(db: Db): Promise<UtilizationDetails> {
    const ie = injectionEffectiveness;
  ...
  async queryAB(db: Db): Promise<ABComparison> {
    const ie = injectionEffectiveness;
  ...
  async queryTrend(db: Db): Promise<EffectivenessTrendPoint[]> {
    const ie = injectionEffectiveness;
