...
export class IntentProjectionView implements ProjectionView<IntentProjectionPayload> {
  ...
  applyEvent(event: ProjectionEvent): boolean {
    ...
    // Deduplicate: skip events whose upstream ID has already been processed.
    // Fallback IDs generated by ProjectionService (proj-N) are per-ingest, so
    // we only dedup IDs that look like real upstream identifiers (non-empty,
    // not the proj-N fallback prefix).
    if (event.id && !event.id.startsWith('proj-')) {
      if (this.seenIds.has(event.id)) return false;
      this.seenIds.add(event.id);
      // Bounded: clear when set grows large to prevent unbounded memory use.
      // After clearing, a brief window of duplicates may slip through — acceptable.
