...
export class AgentExecutionTracker {
  ...
  static getAgentPerformanceMetrics(agentId: string) {
    ...
    const lastUsed = executions.sort(
    ...
    const recentExecutions = executions.filter(
      (exec) => new Date(exec.startedAt) >= last24Hours
    ).length;
    ...
    const efficiency = Math.max(
      0,
      Math.min(100, successRate * 0.7 + (100 - Math.min(avgExecutionTime / 60, 100)) * 0.3)
    );
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const execution: AgentExecution = {
        ...
        completedAt: new Date(
          startedAt.getTime() + (30 + Math.random() * 300) * 1000
        ).toISOString(),
