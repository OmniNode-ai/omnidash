    ...
    try {
      await db.execute(sql`
        INSERT INTO context_enrichment_events (
          ...
          ${outcome},
          ${Number.isNaN(Number(evt.latency_ms)) ? null : Number(evt.latency_ms ?? 0)},
          ${Number.isNaN(Number(evt.tokens_before)) ? null : Number(evt.tokens_before ?? 0)},
          ${Number.isNaN(Number(evt.tokens_after)) ? null : Number(evt.tokens_after ?? 0)},
          ${Number.isNaN(Number(evt.net_tokens_saved)) ? null : Number(evt.net_tokens_saved ?? 0)},
          ${evt.similarity_score != null && !Number.isNaN(Number(evt.similarity_score)) ? Number(evt.similarity_score) : null},
          ${evt.quality_score != null ? Number(evt.quality_score) : null},
          ${(evt.repo as string) ?? null},
          ${(evt.agent_name as string) ?? null},
          ${safeParseDate(evt.timestamp)}
        )
