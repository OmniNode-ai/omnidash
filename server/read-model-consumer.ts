    ...
    try {
      await db.execute(sql`
        INSERT INTO context_enrichment_events (
          ...
          ${outcome},
          ${Number(evt.latency_ms ?? 0)},
          ${Number(evt.tokens_before ?? 0)},
          ${Number(evt.tokens_after ?? 0)},
          ${Number(evt.net_tokens_saved ?? 0)},
          ${evt.similarity_score != null ? Number(evt.similarity_score) : null},
