...
router.get("/timeseries", (req, res) => {
  try {
    ...
    const timeSeriesData = Array.from({ length: days }, (_, i) => {
      ...
      const dayRuns = mockAgentRuns.filter(run => 
      ...
      return {
        ...
        savings: {
          tokens: baselineTokens - intelligenceTokens,
          compute: baselineCompute - intelligenceCompute,
          cost: baselineCost - intelligenceCost,
          percentage: baselineCost > 0 ? ((baselineCost - intelligenceCost) / baselineCost) * 100 : 0,
        },
      };
    });
