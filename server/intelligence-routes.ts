...
// ============================================================================
// Pattern Injection Pipeline Routes (OMN-2191)
// ============================================================================

/**
 * GET /api/intelligence/injections/recent?limit=50
 * List recent pattern injection events with A/B cohort data.
 */
intelligenceRouter.get('/injections/recent', async (req: Request, res: Response) => {
  try {
    const db = getIntelligenceDb();
...
/**
 * GET /api/intelligence/injections/cohort-summary
 * Aggregated A/B cohort comparison: treatment vs control counts, success rates.
 */
intelligenceRouter.get('/injections/cohort-summary', async (req: Request, res: Response) => {
  try {
    const db = getIntelligenceDb();
    if (!db) return res.status(503).json({ error: 'Intelligence DB not available' });
...
/**
 * GET /api/intelligence/lifecycle/recent?limit=50
 * List recent pattern lifecycle transitions (promotions/demotions).
 */
intelligenceRouter.get('/lifecycle/recent', async (req: Request, res: Response) => {
  try {
    const db = getIntelligenceDb();
    if (!db) return res.status(503).json({ error: 'Intelligence DB not available' });
...
/**
 * GET /api/intelligence/lifecycle/summary?timeWindow=7d
 * Aggregated lifecycle transition metrics: promotion/demotion counts by trigger.
 */
intelligenceRouter.get('/lifecycle/summary', async (req: Request, res: Response) => {
  try {
    const db = getIntelligenceDb();
    if (!db) return res.status(503).json({ error: 'Intelligence DB not available' });
...
/**
 * GET /api/intelligence/lifecycle/pattern/:patternId
 * Full lifecycle audit trail for a specific pattern.
 */
intelligenceRouter.get('/lifecycle/pattern/:patternId', async (req: Request, res: Response) => {
  try {
    const db = getIntelligenceDb();
    if (!db) return res.status(503).json({ error: 'Intelligence DB not available' });
...
/**
 * GET /api/intelligence/attributions/recent?limit=50
 * List recent measured attribution records.
 */
intelligenceRouter.get('/attributions/recent', async (req: Request, res: Response) => {
  try {
    const db = getIntelligenceDb();
    if (!db) return res.status(503).json({ error: 'Intelligence DB not available' });
...
/**
 * GET /api/intelligence/attributions/summary?timeWindow=7d
 * Aggregated attribution metrics: tier distribution and pattern coverage.
 */
intelligenceRouter.get('/attributions/summary', async (req: Request, res: Response) => {
  try {
    const db = getIntelligenceDb();
    if (!db) return res.status(503).json({ error: 'Intelligence DB not available' });
