...
function buildAlignedGrid(
  ...
  // Stacked layout: bars all sit at z=0 within a one-cell-deep footprint.
  // The grid is a shallow floor strip — front and back edges along X plus
  // bucket-divider lines perpendicular to time. Per-model row lines (one
  // per model, spanning Z = modelCount * CELL_SIZE) would imply lanes that
  // don't exist here, so they're omitted.
  const bucketCount = dataset.buckets.length;
  const spanX = bucketCount * CELL_SIZE;
  const spanZ = CELL_SIZE;
  const positions: number[] = [];
  // Front + back edges of the floor strip.
  positions.push(-spanX / 2, 0, -spanZ / 2, spanX / 2, 0, -spanZ / 2);
  positions.push(-spanX / 2, 0, +spanZ / 2, spanX / 2, 0, +spanZ / 2);
  // Bucket dividers — vertical lines on the floor, one per bucket boundary.
...
function buildBars(
  ...
  // Stacked-column layout: one column per bucket at z=0; within each column,
  // model contributions stack along Y in modelIndex order so the same model
  // always appears at the same vertical position across buckets.
  const cellsByBucket = new Map<number, GridCell[]>();
  for (const cell of dataset.cells) {
    const arr = cellsByBucket.get(cell.bucketIndex) ?? [];
    arr.push(cell);
    cellsByBucket.set(cell.bucketIndex, arr);
  }

  // Normalize against the tallest stack, not the tallest single cell, so
  // the highest column fills the canvas without clipping. dataset.maxCost
  // (per-cell max) is preserved for callers that still care; stacked
  // rendering needs the per-bucket sum.
  let maxStack = 0;
  for (const cells of cellsByBucket.values()) {
    const sum = cells.reduce((acc, c) => acc + c.cost, 0);
    if (sum > maxStack) maxStack = sum;
  }
  maxStack = Math.max(maxStack, 0.001);

  const barWidth = CELL_SIZE * 0.72;

  for (const cells of cellsByBucket.values()) {
    cells.sort((a, b) => a.modelIndex - b.modelIndex);
    let cumulative = 0;
    for (const cell of cells) {
      const segmentHeight = Math.max(0.02, (cell.cost / maxStack) * MAX_BAR_HEIGHT);
      const yBottom = (cumulative / maxStack) * MAX_BAR_HEIGHT;
      const yMid = yBottom + segmentHeight / 2;
      ...
      // All stacks share z=0; the third dimension is just camera perspective.
      ...
      const core = new THREE.Mesh(
        new THREE.BoxGeometry(barWidth, segmentHeight, barWidth),
        coreMat,
      );
      ...
      const top = new THREE.Mesh(
        new THREE.PlaneGeometry(barWidth, barWidth),
        topMat,
      );
      top.rotation.x = -Math.PI / 2;
      top.position.y = yBottom + segmentHeight + 0.001;
      barGroup.add(top);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (currentActive) {
        ...
        // Stacked layout: a bar segment sits at core.position.y, NOT at
        // height/2. With stacking, segments above the first don't start
        // at y=0 — the Y offset comes from the cumulative stack below.
        // Use core.position.y directly as the focus target.
        const segmentY = core.position.y;
        focusLookAt.set(barPos.x + FOCUS_LOOKAT_X_OFFSET, segmentY, barPos.z);
        focusPos.set(barPos.x, segmentY + FOCUS_ELEVATION, barPos.z + FOCUS_DISTANCE);
      } else {
      ...
      if (ds) {
        ...
        // Stacked layout: bars all sit at z=0 with depth ≈ CELL_SIZE wide.
        // Place bucket labels just in front of the bar fronts. The previous
        // formula (spanZ = modelCount * CELL_SIZE) assumed bars spread along Z
        // by model — they no longer do.
        const labelZ = CELL_SIZE / 2 + 0.45;
        const bucketLabels = ds.buckets.map((bucket, i) => {
          const x = layoutX(i, ds.buckets.length);
          const s = toScreen(new THREE.Vector3(x, 0, labelZ));
          ...
          // day rolls over (avoids a wall of repeated "MM/DD"s on hourly
          // buckets). The rollover check uses the SAME zone so a Tokyo user
          // viewing UTC sees rollovers at UTC midnight, not local midnight.
          let showDate = i === 0;
          if (!showDate) {
        ...
        // Stacked layout: model identity is conveyed by stack order + the
        // legend chips below the canvas. Skipping in-canvas model labels —
        // they would all project to the same screen point (since every stack
        // shares z=0) and visually overlap.
...
export default function CostTrend3D({ config: _config }: { config: Record<string, unknown> }) {
  ...
  //
  // The chart tokens are declared as `oklch(...)` in globals.css. THREE.Color
  // can't parse oklch (and `${color}22` hex-alpha concatenation in the legend
  // chips below also requires `#rrggbb`), so normalize the whole palette to
  // hex strings here via cssColorToHex. Without this conversion, every bar
  // falls back to THREE's default grey.
