...
// not count as removed). These assertions check that CLAUDE.md still names
// the decision's canonical page directly.
const KB_PAGE = 'adrs/ADR-0039-omnidash-typography-system.md';

describe('Phase 0: ADR', () => {
  it('CLAUDE.md links to the canonical ADR page and contains the Typography section', () => {
    const claude = readSrc('CLAUDE.md');
    expect(claude).toContain(KB_PAGE);
...
describe('Phase 6: Docs + cleanup', () => {
  it('root README names the canonical typography primitives page', () => {
    const readme = readSrc('README.md');
    expect(readme, 'README does not name the typography primitives KB page').toContain(
