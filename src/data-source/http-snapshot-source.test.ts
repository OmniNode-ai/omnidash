...
describe('HttpSnapshotSource', () => {
  ...
  it('surfaces a missing projection instead of misreporting an empty stream', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce({
      ok: false,
      status: 404,
    }));
    ...
    const source = new HttpSnapshotSource({ baseUrl: 'http://localhost:3002' });
    await expect(async () => {
      for await (const _item of source.readAll('onex.snapshot.missing.v1')) {
        /* unreachable */
      }
    }).rejects.toThrow(/HTTP 404/);
  });

  it('surfaces a projection outage instead of misreporting an empty stream', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce({
      ok: false,
      status: 500,
    }));
    ...
    const source = new HttpSnapshotSource({ baseUrl: 'http://localhost:3002' });
    await expect(async () => {
      for await (const _item of source.readAll('onex.snapshot.error.v1')) {
        /* unreachable */
      }
