...
export default function NodeNetworkComposer() {
  ...
  React.useEffect(() => {
    if (nodes.length === 0 && edges.length === 0) {
      const mockNodes: ComposerNode[] = [
        { id: "orchestrator-root", name: "root-orchestrator", type: "Orchestrator", contract: "onex://orchestrators/root.yaml", x: 300, y: 120 },
        { id: "compute-ingest", name: "ingest-service", type: "Compute", image: "ghcr.io/org/ingest:latest", x: 120, y: 300 },
        { id: "reducer-aggregate", name: "aggregate-metrics", type: "Reducer", contract: "onex://reducers/aggregate.yaml", x: 300, y: 300 },
        { id: "effect-notify", name: "notify-alerts", type: "Effect", contract: "onex://effects/notify.yaml", x: 480, y: 300 },
      ];
      const mockEdges: ComposerEdge[] = [
        { id: "e1", source: "compute-ingest", target: "reducer-aggregate", channel: "agent-actions" },
        { id: "e2", source: "reducer-aggregate", target: "effect-notify", channel: "router-performance-metrics" },
        { id: "e3", source: "orchestrator-root", target: "compute-ingest", channel: "agent-routing-decisions" },
      ];
      setNodes(mockNodes);
      setEdges(mockEdges);
    }
  ...
  const addEdge = () => {
    ...
    setEdges((prev) => [{ id, source: linkSource, target: linkTarget, channel: linkChannel }, ...prev]);
    setLinkSource("");
    setLinkTarget("");
    setLinkChannel("");
  };
  ...
  const handleNodeMouseUp = useCallback((targetNodeId?: string) => {
  ...
  const yamlExport = useMemo(() => {
    const doc = {
      version: "2.0",
      network: {
        nodes: nodes.map((n) => ({ id: n.id, name: n.name, type: n.type, contract: n.contract, image: n.image })),
        edges: edges.map((e) => ({ id: e.id, source: e.source, target: e.target, channel: e.channel })),
      },
    };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Design full ONEX node networks with drag-and-drop and visual connections.</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => navigator.clipboard.writeText(yamlExport)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="node-name" value={newNodeName} onChange={(e) => setNewNodeName(e.target.value)} placeholder="e.g. auth-service" />
            </div>
            <div className="space-y-2">
              <Label>Type</Label>
            ...
            <div className="space-y-2">
              ...
              <Input id="node-contract" value={newNodeContract} onChange={(e) => setNewNodeContract(e.target.value)} placeholder="e.g. onex://contracts/auth.yaml" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="node-image">Image (optional)</Label>
              <Input id="node-image" value={newNodeImage} onChange={(e) => setNewNodeImage(e.target.value)} placeholder="e.g. ghcr.io/org/service:tag" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input value={linkChannel} onChange={(e) => setLinkChannel(e.target.value)} placeholder="channel/topic (e.g. agent-actions)" />
        ...
        <div className="lg:col-span-3 space-y-6">
              ...
              >
                ...
                {nodes.map((node) => (
                  ...
                  >
                    ...
                    {node.contract && (
                      <div className="text-xs text-muted-foreground truncate mb-1" title={node.contract}>
                        {node.contract}
                      </div>
                    )}
                    {node.image && (
                      <div className="text-xs text-muted-foreground truncate mb-1" title={node.image}>
                        {node.image}
                      </div>
                    )}
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {edges.map((e) => (
                    <div key={e.id} className="border rounded p-2 text-sm flex items-center justify-between">
                      <div>
                        <span className="font-semibold">{nodes.find(n => n.id === e.source)?.name || e.source}</span>
                        <span className="mx-2 text-muted-foreground">→</span>
                        <span className="font-semibold">{nodes.find(n => n.id === e.target)?.name || e.target}</span>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                {validation.ok ? <CheckCircle className="w-5 h-5 text-green-600" /> : <AlertTriangle className="w-5 h-5 text-yellow-600" />}
                Validation
              </CardTitle>
