...
export function Pager({ page, pageSize, totalItems, onPageChange, onPageSizeChange, pageSizeOptions = [10, 20, 50] }: PagerProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      <Button variant="outline" size="sm" disabled={!canPrev} onClick={() => onPageChange(Math.max(1, page - 1))}>
        Prev
      </Button>
      <div className="text-xs text-muted-foreground">
        Page <span className="font-medium">{page}</span>{totalPages ? <> / <span className="font-medium">{totalPages}</span></> : null}
      </div>
