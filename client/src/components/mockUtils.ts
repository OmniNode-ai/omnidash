...
export function generateTimeSeries(count = 20, base = 1, variance = 0.4): TimeSeriesPoint[] {
  ...
  for (let i = count - 1; i >= 0; i--) {
    const t = new Date(now - i * 60 * 1000).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
...
export function generateCategorical(entries: Array<{ key: string }>, total = 100): Array<{ key: string; value: number; percentage: number }> {
...
export function ensureTimeSeries(data?: TimeSeriesPoint[], base = 1, variance = 0.4): { data: TimeSeriesPoint[]; isMock: boolean } {
  if (data && data.length > 0) return { data, isMock: false };
  return { data: generateTimeSeries(20, base, variance), isMock: true };
}

export function ensureArray<T>(items: T[] | undefined, fallback: T[]): { data: T[]; isMock: boolean } {
