...
export function EventBusHealthIndicator({
  className,
  showLabel = true,
}: EventBusHealthIndicatorProps) {
  const {
    data: status,
    isLoading,
    isError,
  ...
  const effectiveStatus: EventBusStatus['status'] = isError ? 'error' : status?.status || 'unknown';
    ...
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent>
          ...
          {(effectiveStatus === 'stopped' || effectiveStatus === 'error') && (
            <p className="text-destructive">Check server logs for errors.</p>
          )}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
