...
- ☐ **Design** — inline style objects match exactly. Wrapper `<div>`: `padding:"20px 12px"`, `fontSize:12`, `color:"var(--sidebar-ink-2)"`, `textAlign:"center"`, `lineHeight:1.5`. Button: `color:"var(--accent)"` (or `"var(--brand)"` per the documented token rename), `textDecoration:"underline"`, `marginTop:4`.
