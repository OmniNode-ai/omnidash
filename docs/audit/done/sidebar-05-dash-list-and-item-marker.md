...
All prototype rules for `.dash-list`, `.dash-item` (including `:hover` and `.active`), `.dash-item .dash-marker`, and `.dash-item.active .dash-marker` are present at `src/styles/sidebar.css:70-106` with identical values. The `var(--accent)` → `var(--brand)` substitution at line 104 is the documented intentional rename (called out in the file header at lines 2-3), not a finding.
...
No changes needed — chunk audited clean across all three axes; marker glyph and dashboard-item wiring match prototype.
