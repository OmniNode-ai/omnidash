...
Note: prototype uses `var(--accent)` in the `.lib-card:hover` rule; v2 renamed that token to `var(--brand)` as a documented intentional rename. The substitution is not a finding. `.lib-card-thumb` uses `var(--accent-ink)` which is a separate token — verify v2's rename mapping.
...
- v2: `ComponentPalette.tsx:53-67` omits `draggable`, `onDragStart`, and `onDragEnd` entirely; the `ComponentPaletteProps` interface at `ComponentPalette.tsx:17-20` exposes only `onAddComponent` with no drag callbacks.
