...
Prototype signature is `(w, i) => …` so the index is available for the `dragOverSlot === i` guard. v2 is `(item) => …` with no index. Consistent with the dropped DropIndicator, but worth noting for the the drag-and-drop work restoration.
...
Empty-state branch resolved in commit `4145b96` (`<EmptyState onAdd={handleEdit}/>`); onAdd wired correctly. React.Fragment wrapping + DropIndicator + dragOverSlot guard are a future drag-and-drop scope (drag-and-drop); related blocked chunks: widget-card-01/02/04, dashboard-08/09/10.
