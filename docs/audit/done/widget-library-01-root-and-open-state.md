...
Note: prototype uses `var(--accent)` elsewhere in the library block; v2 renamed that token to `var(--brand)` as a documented intentional rename. The substitution is not a finding.
...
- v2: `src/components/dashboard/ComponentPalette.tsx:36` hardcodes `className="library open"` as a string literal. There is no prop, state, or conditional driving the `open` class. The file-level comment at `src/components/dashboard/ComponentPalette.tsx:7` also documents "No … slide-in animation yet" as a deliberate deferral, confirming this is intentional, not an accidental omission.
...
Accepted v2 architectural deviation — `className="library open"` is hardcoded because v2 conditionally renders the palette (editMode toggles mount/unmount) rather than using the prototype's CSS slide-in transition. The close button was wired in a prior pass (commit `4145b96`), so the rail can be dismissed. The `.library` transform rule is unused by design in v2.
