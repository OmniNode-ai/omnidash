...
Note: prototype uses `var(--accent)` in the `.btn.primary`/`.btn.accent` rules; v2 renamed that token to `var(--brand)` in an earlier token-rename pass. The substitution is not a finding — it's a documented intentional rename. Same applies to `--accent-h`, `--accent-soft`, `--accent-ink` if those appear in the rules.
