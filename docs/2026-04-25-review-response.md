...
|---|---|---|---|
| PR #33 | env-var sweep + I/O validation + WS carve-out + grep gate | T1–T5 | `9a14197` |
| PR #34 | capability/test honesty + server typecheck discipline | T6, T7, T9–T13 | `f3e85a0` |
| PR #35 | truth-ownership refactor — service-led writes + source context + topic SoT | T14–T16 | `18a7651` |
| PR #36 | 2D companions + threshold config + cleanup bundle | T17–T21 | `7a0beb3` |
...
**Status:** Fixed in PR 1 (T4).
**Where:** `src/hooks/useWebSocketInvalidation.ts` (subsequently removed when the `/ws` invalidation path was found dead end-to-end; the projection backend never registered a `/ws` route and the Express bridge never broadcast).
...
**Where:** [`server/routes.test.ts`](../server/routes.test.ts) (new), `server/broadcast.test.ts` (new; subsequently removed along with the dead `/ws` bridge), [`server/index.ts`](../server/index.ts).
**What changed:**
...
**Where:** `server/broadcast.test.ts` (subsequently removed along with the dead `/ws` bridge).
...
| LOW.6 | `DashboardBuilder.tsx` backwards-compat shim | **Fixed in Bundle 1** (`6c2a9c6`). The shim was a re-export of `DashboardView` for backwards-compat; deleted the shim file and the orphan `DashboardBuilder.css.ts`. Migrated `integration.part2/3.test.tsx` to import `DashboardView` directly. Renamed `DashboardBuilder.test.tsx` → `DashboardView.test.tsx` with consistent describe-block + helper renames. |
