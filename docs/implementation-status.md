...
| Live data updates | Live updates are poll-driven via `useProjectionQuery`. The `/ws` WebSocket path was permanently removed (it was rejected with 403 by the projection backend and never delivered events). Raw browser WebSocket construction is blocked by the `local/no-projection-websocket` ESLint rule. |
