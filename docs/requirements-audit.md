...
| Doc | Status | Used? |
|---|---|---|
| `2026-04-17-dashboard-local-integration.md` (52 KB) | **Authoritative spec** — drove the full engagement task list | **Yes** |
...
of that is omnidash-v2. The dashboard builder project was kicked off two days later with a
...
| 7 | No new hardcoded `localhost:3002` references in `src/` outside the data-source factory carve-out | ⚠ **Two pre-engagement holdovers remain** — `src/components/dashboard/events/EventStream.tsx:24` and `src/hooks/useWebSocketInvalidation.ts:17` both reference `ws://localhost:3002/ws`. These predate Part 3; the fix is to introduce a `VITE_WS_DATA_SOURCE_URL` carve-out for the WebSocket bridge. The HTTP carve-out itself is properly contained inside `src/data-source/index.ts`. |
...
All 12 tasks (Tasks 1–12 from the plan). Spot-checked file-by-file:
...
|---|---|
| Filter `index.json` out of `fixturesMiddleware` synthetic topic listing | Open (latent — generator never writes `index.json`, so no observable bug today). |
| `VITE_WS_DATA_SOURCE_URL` carve-out for the WebSocket bridge | Open (paired with the two `localhost:3002` holdovers in Acceptance #7). |

---

## What's been delivered since the engagement report

The 04-19 report covered the original Part 3 plan (Tasks 1–12). After that, a
substantial amount of polish, refactor, and feature work landed. Summary, in rough chronological order:

- **Day-2 fixes** — "+ New dashboard" button in header; live `RegistryProvider` calls `resolveImplementations()`. Both merged.
- **UX polish pass.** Verbatim CSS port from prototype, sidebar/footer cleanup, widget chrome polish, drag-and-drop drop-slots, theme wiring, masonry grid reflow, routing-table search/sort/paginate (the widget-level features Jonah's prototype implied but the original plan didn't specify).
- **Typography system refactor.** Design tokens, `<Text>`/`<Heading>` primitives, ESLint rule (`local/no-typography-inline`), permanent compliance scorecard. ADR at `docs/adr/001-typography-system.md`.
- **Storybook widget-coverage.** Decorator + fixtures + per-widget stories with Empty/Loading/Error/Populated state coverage. Permanent compliance scorecard. ADR at `docs/adr/002-storybook-widget-coverage.md`.
- **Late polish for handoff.** AgentChat / Sidebar / FrameLayout / DeleteDashboardDialog / ComponentConfigPanel stories; timezone selector; auto-refresh selector with live countdown; topbar Refresh button; delete confirmation; 3D doughnut for DelegationMetrics; collapsible left rail; widget palette regroup by domain.
...
| Item | Source | Status |
|---|---|---|
| `index.json` filter in fixtures middleware | Engagement report follow-up | Open, latent |
| WebSocket URL carve-out (also satisfies Acceptance #7) | Engagement report follow-up | Open |
| AgentLauncher (AI orb) visual redesign | Filed during late-polish story coverage | Open, backlog |
| Responsive design pass for smaller viewports | Filed earlier this week | Canceled per Bret's call |
...
The original engagement (Tasks 1–12 of the 04-17 plan) is **delivered and verified** per the engagement report and re-spot-checked here.
...
- **Hardcoded `localhost:3002` count** — two pre-engagement WebSocket-URL
  holdovers remain. Not produced by Part 3 work; tracked as an open follow-up.
...
project**, not part of the dashboard builder engagement, and should not be cited as
