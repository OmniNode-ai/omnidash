...
| Doc | Status | Used? |
|---|---|---|
| `2026-04-17-dashboard-local-integration.md` (52 KB) | **Authoritative spec for the UX polish epic** — drove every Task ticket a related UX task → the drag-and-drop task | **Yes** |
...
of that is omnidash-v2. The the UX polish epic epic was kicked off two days later with a
...
| 7 | No new hardcoded `localhost:3002` references in `src/` outside the data-source factory carve-out | ⚠ **Two pre-engagement holdovers remain** — `src/components/dashboard/events/EventStream.tsx:24` and `src/hooks/useWebSocketInvalidation.ts:17` both reference `ws://localhost:3002/ws`. These predate Part 3; tracked as **the WebSocket task** ("Introduce `VITE_WS_DATA_SOURCE_URL` carve-out for the WebSocket bridge"). The HTTP carve-out itself is properly contained inside `src/data-source/index.ts`. |
...
a related UX task → the drag-and-drop task. Spot-checked file-by-file:
...
|---|---|
| **the responsive layout task** — Filter `index.json` out of `fixturesMiddleware` synthetic topic listing | Open (latent — generator never writes `index.json`, so no observable bug today). |
| **the WebSocket task** — `VITE_WS_DATA_SOURCE_URL` carve-out for the WebSocket bridge | Open (paired with the two `localhost:3002` holdovers in Acceptance #7). |

---

## What's been delivered since the engagement report

The 04-19 report covered a related UX task → the drag-and-drop task (the Part 3 plan). After that, a
substantial amount of polish, refactor, and feature work landed under the
same the UX polish epic umbrella. Summary, in rough chronological order:

- **Day-2 fixes (the new-dashboard-button task / the registry-resolve task)** — "+ New dashboard" button in header; live `RegistryProvider` calls `resolveImplementations()`. Both merged.
- **a related registry task → the color system task — UX polish pass.** Verbatim CSS port from prototype, sidebar/footer cleanup, widget chrome polish, drag-and-drop drop-slots, theme wiring, masonry grid reflow, routing-table search/sort/paginate (the widget-level features Jonah's prototype implied but the original plan didn't specify).
- **the typography refactor → the compliance task — Typography system refactor.** Design tokens, `<Text>`/`<Heading>` primitives, ESLint rule (`local/no-typography-inline`), permanent compliance scorecard. ADR at `docs/adr/001-typography-system.md`.
- **the Storybook coverage epic → a related task — Storybook widget-coverage.** Decorator + fixtures + per-widget stories with Empty/Loading/Error/Populated state coverage. Permanent compliance scorecard. ADR at `docs/adr/002-storybook-widget-coverage.md`.
- **a related task → a related task — Late polish for handoff.** AgentChat / Sidebar / FrameLayout / DeleteDashboardDialog / ComponentConfigPanel stories; timezone selector; auto-refresh selector with live countdown; topbar Refresh button; delete confirmation; 3D doughnut for DelegationMetrics; collapsible left rail; widget palette regroup by domain.
...
| Item | Source | Status |
|---|---|---|
| the responsive layout task — `index.json` filter in fixtures middleware | Engagement report follow-up | Open, latent |
| the WebSocket task — WebSocket URL carve-out (also satisfies Acceptance #7) | Engagement report follow-up | Open |
| a related task — AgentLauncher (AI orb) visual redesign | Filed during a related task story coverage | Open, backlog |
| a related task — Responsive design pass for smaller viewports | Filed earlier this week | Canceled per Bret's call |
...
The original engagement (Tasks 1–12 of the 04-17 plan, a related UX task through
the drag-and-drop task) is **delivered and verified** per the engagement report and
...
- **Hardcoded `localhost:3002` count** — two pre-engagement WebSocket-URL
  holdovers remain. Tracked as the WebSocket task; not produced by Part 3 work.
...
project**, not part of the UX polish epic, and should not be cited as
