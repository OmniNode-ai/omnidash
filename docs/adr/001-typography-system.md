...
**Date:** 2026-04-24
**Authors:** OmniNode-ai
**Related:** Typography System Refactor epic, `docs/plans/typography-refactor.md`

## Context

During a UX polish pass, two dashboard widgets — `EventStream` and `RoutingDecisionTable` — were observed to diverge visually despite being designed to read as a matched pair of "data feed" surfaces. The symptoms were subtle but systematic:
...
| Compliance file excluded from default run | Refactor doesn't red the CI while in progress | Easy to forget to re-enable at Task 39 | Task 39's acceptance explicitly checks the `exclude` list and gates promotion |
| 39-ticket epic | Broken into waves; multi-agent workflow can parallelize the 17 widget migrations | Each ticket adds Linear noise | Old work tracked under the typography epic umbrella without a sprawling sibling-epic set |
...
- Epic: Typography System Refactor
